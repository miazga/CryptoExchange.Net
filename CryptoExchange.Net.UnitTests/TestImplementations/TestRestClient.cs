...
public class TestRestClient : BaseRestClient
{
    ...
    public void SetResponse(string responseData, out IRequest requestObj)
    {
        ...
        request.Setup(c => c.SetContent(It.IsAny<string>(), It.IsAny<string>())).Callback(
            new Action<string, string>((content, type) => { request.Setup(r => r.Content).Returns(content); }));
        request.Setup(c => c.AddHeader(It.IsAny<string>(), It.IsAny<string>()))
            .Callback<string, string>((key, val) => headers.Add(key, new List<string> { val }));
        request.Setup(c => c.GetHeaders()).Returns(() => headers);

        Mock<IRequestFactory> factory = Mock.Get(RequestFactory);
    ...
    {
        HttpRequestException we = new HttpRequestException();
        typeof(HttpRequestException)
            .GetField("_message", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
            .SetValue(we, message);
    ...
    public async Task<CallResult<T>> RequestWithParams<T>(HttpMethod method, Dictionary<string, object> parameters,
        Dictionary<string, string> headers) where T : class
    {
        return await SendRequestAsync<T>(Api1, new Uri("http://www.test.com"), method, default, parameters,
...
public class TestRestApi1Client : RestApiClient
{
    ...
    {
        return new TestAuthProvider(credentials);
    }
...
public class TestAuthProvider : AuthenticationProvider
{
        ...
        out SortedDictionary<string, object> bodyParameters, out Dictionary<string, string> headers)
    {
        uriParameters = parameterPosition == HttpMethodParameterPosition.InUri
            ? new SortedDictionary<string, object>(providedParameters)

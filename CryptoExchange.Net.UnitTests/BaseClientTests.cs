...
public class BaseClientTests
{
    ...
    {
        // arrange
        TestStringLogger logger = new TestStringLogger();
        TestBaseClient client = new TestBaseClient(new BaseRestClientOptions
        {
    ...
    public void SettingLogLevel_Should_RestrictLogging(LogLevel? verbosity, LogLevel testVerbosity, bool expected)
    {
        // arrange
        TestStringLogger logger = new TestStringLogger();
        BaseRestClientOptions options = new BaseRestClientOptions { LogWriters = new List<ILogger> { logger } };
        if (verbosity != null)
        {
            options.LogLevel = verbosity.Value;
        }

        TestBaseClient client = new TestBaseClient(options);
    ...
    {
        // arrange
        TestBaseClient client = new TestBaseClient();

        // act
        CallResult<object> result = client.Deserialize<object>("{\"testProperty\": 123}");
    ...
    {
        string result = baseUrl.AppendPath(path);
